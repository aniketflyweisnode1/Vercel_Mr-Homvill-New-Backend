const Properties_Renter_hub = require('../models/Properties_Renter_hub.model');
const Properties = require('../models/Properties.model');
const User = require('../models/User.model');

// Create Properties_Renter_hub (auth)
const createPropertiesRenterHub = async (req, res) => {
  try {
    ...
    const item = new Properties_Renter_hub({
    ...
    res.status(201).json({ success: true, message: 'Properties renter hub created successfully', data: response });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error creating properties renter hub', error: error.message });
  }
};

// Get all (public)
const getAllPropertiesRenterHub = async (req, res) => {
  try {
    const items = await Properties_Renter_hub.find({ Status: true }).sort({ CreateAt: -1 });
    ...
    res.status(500).json({ success: false, message: 'Error fetching properties renter hub', error: error.message });
  }
};

// Get by id (auth)
const getPropertiesRenterHubById = async (req, res) => {
  try {
    const { id } = req.params;
    const item = await Properties_Renter_hub.findOne({ Properties_Renter_hub_id: parseInt(id) });
    if (!item) {
      return res.status(404).json({ success: false, message: 'Properties renter hub not found' });
    }
    const [createByUser, updatedByUser, propertiesData, savedUser] = await Promise.all([
      item.CreateBy ? User.findOne({ user_id: item.CreateBy }) : null,
      item.UpdatedBy ? User.findOne({ user_id: item.UpdatedBy }) : null,
...
const getPropertiesRenterHubByAuth = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const items = await Properties_Renter_hub.find({ user_id: userId, Status: true }).sort({ CreateAt: -1 });
    const response = await Promise.all(items.map(async (item) => {
      const [createByUser, updatedByUser, propertiesData, savedUser] = await Promise.all([
        item.CreateBy ? User.findOne({ user_id: item.CreateBy }) : null,
        item.UpdatedBy ? User.findOne({ user_id: item.UpdatedBy }) : null,
    ...
    res.status(500).json({ success: false, message: 'Error fetching properties renter hub by auth', error: error.message });
  }
};

// Update (auth)
const updatePropertiesRenterHub = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const { id, ...updateData } = req.body;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Properties_Renter_hub_id is required in request body' });
    }
    const item = await Properties_Renter_hub.findOne({ Properties_Renter_hub_id: parseInt(id) });
    if (!item) {
      return res.status(404).json({ success: false, message: 'Properties renter hub not found' });
    ...
    Object.keys(updateData).forEach((key) => {
      if (key !== 'Properties_Renter_hub_id') {
    ...
    res.status(200).json({ success: true, message: 'Properties renter hub updated successfully', data: response });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error updating properties renter hub', error: error.message });
  }
};

// Delete (auth)
const deletePropertiesRenterHub = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const { id } = req.params;
    const item = await Properties_Renter_hub.findOne({ Properties_Renter_hub_id: parseInt(id) });
    ...
    res.status(200).json({ success: true, message: 'Properties renter hub deleted successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error deleting properties renter hub', error: error.message });
  }
};
