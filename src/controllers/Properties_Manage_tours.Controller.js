const Properties_Manage_tours = require('../models/Properties_Manage_tours.model');
const Properties = require('../models/Properties.model');
const User = require('../models/User.model');

// Create Properties_Manage_tours (auth)
const createPropertiesManageTours = async (req, res) => {
  try {
    ...
    const item = new Properties_Manage_tours({
    ...
    res.status(201).json({ success: true, message: 'Properties manage tours created successfully', data: response });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error creating properties manage tours', error: error.message });
  }
};

// Get all (public)
const getAllPropertiesManageTours = async (req, res) => {
  try {
    console.log('getAllPropertiesManageTours');
    const items = await Properties_Manage_tours.find({ Status: true }).sort({ CreateAt: -1 });
    ...
    res.status(500).json({ success: false, message: 'Error fetching properties manage tours', error: error.message });
  }
};

// Get by id (auth)
const getPropertiesManageToursById = async (req, res) => {
  try {
    const { id } = req.params;
    const item = await Properties_Manage_tours.findOne({ Properties_Manage_tours_id: parseInt(id) });
    if (!item) {
      return res.status(404).json({ success: false, message: 'Properties manage tours not found' });
    }
    const [createByUser, updatedByUser, propertiesData, savedUser] = await Promise.all([
      item.CreateBy ? User.findOne({ user_id: item.CreateBy }) : null,
      item.UpdatedBy ? User.findOne({ user_id: item.UpdatedBy }) : null,
...
const getPropertiesManageToursByAuth = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const items = await Properties_Manage_tours.find({ user_id: userId, Status: true }).sort({ CreateAt: -1 });
    const response = await Promise.all(items.map(async (item) => {
      const [createByUser, updatedByUser, propertiesData, savedUser] = await Promise.all([
        item.CreateBy ? User.findOne({ user_id: item.CreateBy }) : null,
        item.UpdatedBy ? User.findOne({ user_id: item.UpdatedBy }) : null,
    ...
    res.status(500).json({ success: false, message: 'Error fetching properties manage tours by auth', error: error.message });
  }
};

// Update (auth)
const updatePropertiesManageTours = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const { id, ...updateData } = req.body;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Properties_Manage_tours_id is required in request body' });
    }
    const item = await Properties_Manage_tours.findOne({ Properties_Manage_tours_id: parseInt(id) });
    if (!item) {
      return res.status(404).json({ success: false, message: 'Properties manage tours not found' });
    ...
    Object.keys(updateData).forEach((key) => {
      if (key !== 'Properties_Manage_tours_id') {
    ...
    res.status(200).json({ success: true, message: 'Properties manage tours updated successfully', data: response });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error updating properties manage tours', error: error.message });
  }
};

// Delete (auth)
const deletePropertiesManageTours = async (req, res) => {
  try {
    const userId = req.user.user_id;
    const { id } = req.params;
    const item = await Properties_Manage_tours.findOne({ Properties_Manage_tours_id: parseInt(id) });
    ...
    res.status(200).json({ success: true, message: 'Properties manage tours deleted successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error deleting properties manage tours', error: error.message });
  }
};
